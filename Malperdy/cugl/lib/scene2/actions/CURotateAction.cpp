 ...
 */
void RotateBy::update(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state, float dt) {
 ...
 * @param state     The relevant node state
 */
void RotateTo::load(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state) {
 ...
 * @param dt        The elapsed time to animate.
 */
void RotateTo::update(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state, float dt) {
    float angle = target->getAngle();
