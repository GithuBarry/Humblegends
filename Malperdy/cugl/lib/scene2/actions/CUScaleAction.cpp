 ...
 */
void ScaleBy::load(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state) {
 ...
 */
void ScaleBy::update(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state, float dt) {
 ...
 * @param state     The relevant node state
 */
void ScaleTo::load(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state) {
    Vec2* scale = (Vec2*)state;
    *scale = target->getScale();
}
 ...
 * @param dt        The elapsed time to animate.
 */
void ScaleTo::update(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state, float dt) {
    Vec2 scale = target->getScale();
    Vec2* orig = (Vec2*)state;
