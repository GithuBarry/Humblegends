 ...
 */
void Animate::load(const std::shared_ptr<SceneNode>& target, Uint64* state) {
    SpriteNode* strip = dynamic_cast<SpriteNode*>(target.get());
 ...
 */
void Animate::update(const std::shared_ptr<SceneNode>& target, Uint64* state, float dt) {
    SpriteNode* strip = dynamic_cast<SpriteNode*>(target.get());
    CUAssertLog(strip,"Attempt to animate a node other than an AnimationNode");
