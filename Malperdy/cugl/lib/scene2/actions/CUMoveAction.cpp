 ...
 */
void MoveBy::update(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state, float dt) {
 ...
 * @param state     The relevant node state
 */
void MoveTo::load(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state) {
 ...
 * @param dt        The elapsed time to animate.
 */
void MoveTo::update(const std::shared_ptr<scene2::SceneNode>& target, Uint64* state, float dt) {
    Vec2  pos  = target->getPosition();
