 ...
 * children of each sub tree are ordered sequentially. To get around this
 * restriction, you must use the {@link OrderedNode} subclass.
 *
...
#pragma mark Size
     ...
     * It is possible for children of this node to be outside of the content
     * size bounds. The concept of content size is primarily for the purpose
     * of anchoring and registering touch input.
     *
     ...
     *
     * In this class, the layout bounds start at the origin of the node and
     * extend to the {@link #getContentSize}. This value can be overridden in
     * subclasses to decouple the layout bounds from the content size (such as
     * nodes that do not show all available content).
     */
    virtual Rect getLayoutBounds() const {
        return Rect(Vec2::ZERO, getContentSize());
    }
     ...
     * contains the transformed node in its parents coordinate system. If
     * the node is rotated, this may not be a perfect fit of the transformed
     * contents.
     *
...
#pragma mark Anchors
     ...
     */
    virtual void setAnchor(float x, float y) { setAnchor(Vec2(x,y)); }
...
#pragma mark Visibility
     ...
     */
    bool hasRelativeColor() const { return _hasParentColor; }
...
#pragma mark Transforms
     ...
     */
    const Affine2& getTransform() const { return _combined; }
     ...
     * left corner).
     *
     * Scaling/rotation and the alternate transform do not play nice with each
     ...
     */
    const Affine2& getAlternateTransform() const { return _transform; }
     ...
     */
    const Affine2& getNodeToParentTransform() const { return _combined; }
     ...
     */
    Affine2 getParentToNodeTransform() const { return _combined.getInverse(); }
